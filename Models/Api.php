<?php namespace Foostart\Api\Models;

use Foostart\Category\Library\Models\FooModel;
use Illuminate\Database\Eloquent\Model;

class Api extends FooModel {
            ...
            foreach($params as $column => $value)
            {
                if($this->isValidValue($value))
                {
                    switch($column)
                    {
                        case 'api_name':
                            if (!empty($value)) {
                                $elo = $elo->where($this->table . '.api_name', '=', $value);
                            }
                            break;
                        case 'status':
                            if (!empty($value)) {
                        ...
                        case 'keyword':
                            if (!empty($value)) {
                                $elo = $elo->where(function($elo) use ($value) {
                                    $elo->where($this->table . '.api_name', 'LIKE', "%{$value}%")
                                    ->orWhere($this->table . '.api_description','LIKE', "%{$value}%")
                                    ->orWhere($this->table . '.api_overview','LIKE', "%{$value}%");
                                });
                            }
                            break;
                        default:
