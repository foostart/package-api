...
use Foostart\Api\Validators\ApiValidator;


class ApiAdminController extends FooController {
    ...
    public function __construct() {
        ...
        parent::__construct();
        // models
        $this->obj_item = new Api(array('perPage' => 10));
        $this->obj_category = new Category();

        // validators
        $this->obj_validator = new ApiValidator();

        // set language files
        $this->plang_admin = 'api-admin';
        $this->plang_front = 'api-front';

        // package name
        $this->package_name = 'package-api';
        $this->package_base_name = 'api';

        // root routers
        $this->root_router = 'apis';
    ...
    public function config(Request $request) {
        $is_valid_request = $this->isValidRequest($request);
        // display view
        $config_path = realpath(base_path('config/package-api.php'));
        $package_path = realpath(base_path('vendor/foostart/package-api'));
        ...
        if ($request->isMethod('post') && $is_valid_request) {
            ...
            file_put_contents($config_bakup.'/package-api-'.date('YmdHis',time()).'.php', $content);
    ...
    public function lang(Request $request) {
        $is_valid_request = $this->isValidRequest($request);
        // display view
        $langs = config('package-api.langs');
        $lang_paths = [];

        if (!empty($langs) && is_array($langs)) {
            foreach ($langs as $key => $value) {
                $lang_paths[$key] = realpath(base_path('resources/lang/'.$key.'/api-admin.php'));
            }
        }
        ...
        if ($request->isMethod('post') && $is_valid_request) {
            ...
            foreach ($lang_paths as $key => $value) {
                ...
                //format file name api-admin-YmdHis.php
                file_put_contents($lang_bakup.'/'.$key.'/api-admin-'.date('YmdHis',time()).'.php', $content);
            }
